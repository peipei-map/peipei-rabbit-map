...
function statusImage(s){
  return s==="已攻略"?"./images/done.png":
         s==="預約中"?"./images/reserve.png":
         s==="待確認"?"./images/pending.png":
         s==="未攻略"?"./images/todo.png":
         "./images/pending.png";
}
...
function icon(p){
  return L.divIcon({
    html:`<div class="markerWrap"><img class="markerImg" src="${statusImage(p.status)}" alt="${p.status}"><span class="statusLabel ${labelClass(p.status)}">${p.status}</span><span class="markerNum">${p.id}</span></div>`,
    className:"",
    iconSize:[96,82],
    iconAnchor:[32,42],
    popupAnchor:[0,-40]
...
function renderMarkers(){
  ...
  for(const p of places){
    ...
    m.bindPopup(`<b>${p.id}. ${p.name}</b><br>狀態：${p.status}<br>📍 ${p.address}<br>☎️ ${p.phone||"待補"}<br>🐰 ${p.rabbit_note||""}<br><br><a href="${p.google_maps}" target="_blank">開啟 Google 地圖</a>`);
    m.on("click",()=>$("card-"+p.id)?.scrollIntoView({behavior:"smooth",block:"center"}));
    markers[p.id]=m;
  }
}
window.fitAll=()=>{
...
function countStatus(s){return places.filter(p=>p.status===s).length}
function stats(){
  const done=countStatus("已攻略"), pending=countStatus("待確認"), todo=countStatus("未攻略");
...
window.renderCards=()=>{
  const reg=$("region").value, st=$("status").value, q=$("search").value.trim().toLowerCase();
  $("cards").innerHTML="";
  places
    ...
    .forEach(p=>{
      ...
      const sel=edit?`<select onchange="changeStatus('${p.id}',this.value)"><option ${p.status==="待確認"?"selected":""}>待確認</option><option ${p.status==="預約中"?"selected":""}>預約中</option><option ${p.status==="已攻略"?"selected":""}>已攻略</option><option ${p.status==="未攻略"?"selected":""}>未攻略</option></select>`:"";
      const tmpl=edit?`<button onclick="fillTemplate('${p.id}')">套用模板</button>`:"";
      const d=document.createElement("div");
      d.className="card";d.id="card-"+p.id;
      d.innerHTML=`<div class="thumb"><img src="${statusImage(p.status)}" alt="${p.status}"></div><div class="cardBody"><span class="pill ${pill(p.status)}">${p.status}</span> ${sel}<h3>${p.id}. ${p.name}</h3><div class="meta">📍 ${p.address}<br>☎️ ${p.phone||"待補"}<br>🐰 ${p.rabbit_note||""}</div></div><div class="actions"><a class="link" href="${p.google_maps}" target="_blank">Google 地圖</a><a class="link" href="${p.source}" target="_blank">來源</a>${tmpl}<button onclick="focusMarker('${p.id}')">定位圖釘</button></div>`;
      $("cards").appendChild(d);
    });
}
window.focusMarker=id=>{
  const p=places.find(x=>String(x.id)===String(id));if(!p)return;
  map.setView([+p.lat,+p.lng],14);markers[p.id]?.openPopup();
}
window.changeStatus=async(id,status)=>{
  if(!isAdmin)return alert("只有管理員可以修改");
  await updateDoc(doc(db,"restaurants",String(id).padStart(3,"0")),{status});
}
window.seedData=async()=>{
  if(!isAdmin)return alert("請先用管理員帳號登入");
  if(!confirm("匯入 7 筆餐廳資料？"))return;
  for(const p of seedRestaurants){const id=String(p.id).padStart(3,"0");await setDoc(doc(db,"restaurants",id),{...p,id});}
  alert("匯入完成");
}
...
window.fillTemplate=id=>{const p=places.find(x=>String(x.id)===String(id));if(p)$("template").value=template(p);}
...
window.copyPublicTemplate=()=>{
  ...
  navigator.clipboard?.writeText(text).then(()=>alert("Threads 模板已複製")).catch(()=>alert(text));
}
window.downloadRecap=()=>{alert("年度回顧圖卡功能保留中，下一版可再美化輸出。");}
function authBox(){
  if(!IS_ADMIN)return;
  $("auth").innerHTML=!user?`<button onclick="loginGoogle()">Google 登入</button>`: isAdmin?`管理員登入中：${user.email} <button onclick="logoutGoogle()">登出</button>`:`目前登入：${user.email}（非管理員） <button onclick="logoutGoogle()">登出</button>`;
  $("seed").classList.toggle("hidden",!isAdmin);
}
...
},e=>{$("cards").innerHTML=`<p>讀取 Firestore 失敗：${e.message}</p>`;});
