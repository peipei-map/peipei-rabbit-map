...
function color(s){return s==="已攻略"?"#48aa63":s==="未攻略"?"#ef7f1a":"#2d86d1"}
function pill(s){return s==="已攻略"?"pdone":s==="未攻略"?"ptodo":"ppending"}
function icon(p){return L.divIcon({html:`<div class="customMarker" style="background:${color(p.status)}">${p.id}</div>`,className:"",iconSize:[36,36],iconAnchor:[18,18],popupAnchor:[0,-20]})}
...
window.renderCards=()=>{const reg=$("region").value,st=$("status").value,q=$("search").value.trim().toLowerCase();$("cards").innerHTML="";places.filter(p=>(!reg||p.region===reg)&&(!st||p.status===st)).filter(p=>!q||[p.name,p.city,p.address,p.rabbit_note].join(" ").toLowerCase().includes(q)).forEach(p=>{const edit=IS_ADMIN&&isAdmin;const sel=edit?`<select onchange="changeStatus('${p.id}',this.value)"><option ${p.status==="待確認"?"selected":""}>待確認</option><option ${p.status==="預約中"?"selected":""}>預約中</option><option ${p.status==="已攻略"?"selected":""}>已攻略</option><option ${p.status==="未攻略"?"selected":""}>未攻略</option></select>`:"";const tmpl=edit?`<button onclick="fillTemplate('${p.id}')">套用模板</button>`:"";const d=document.createElement("div");d.className="card";d.id="card-"+p.id;d.innerHTML=`<div class="thumb">🐰</div><div><span class="pill ${pill(p.status)}">${p.status}</span> ${sel}<h3>${p.id}. ${p.name}</h3><div>📍 ${p.address}<br>☎️ ${p.phone||"待補"}<br>🐰 ${p.rabbit_note||""}</div><div class="actions"><a class="link" href="${p.google_maps}" target="_blank">Google 地圖</a><a class="link" href="${p.source}" target="_blank">來源</a>${tmpl}<button onclick="focusMarker('${p.id}')">定位圖釘</button></div></div>`;$("cards").appendChild(d)})}
...
window.downloadRecap=()=>{alert("V2.1 已保留年度回顧位置，下一版可再美化圖卡")}
