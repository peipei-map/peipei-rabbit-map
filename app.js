...
function stats(){const done=countStatus("已攻略"),reserved=countStatus("預約中"),pending=countStatus("待確認"),todo=countStatus("未攻略"),r=places.length?done/places.length*100:0;$("done").textContent=done;$("reserved").textContent=reserved;$("pending").textContent=pending;$("todo").textContent=todo;$("total").textContent=places.length;$("bar").style.width=r+"%"}
window.renderCards=()=>{const reg=$("region").value,st=$("status").value,q=$("search").value.trim().toLowerCase();$("cards").innerHTML="";places.filter(p=>(!reg||p.region===reg)&&(!st||p.status===st)).filter(p=>!q||[p.name,p.city,p.address,p.rabbit_note].join(" ").toLowerCase().includes(q)).forEach(p=>{const edit=IS_ADMIN&&isAdmin;const sel=edit?`<select onchange="changeStatus('${p.id}',this.value)"><option ${p.status==="待確認"?"selected":""}>待確認</option><option ${p.status==="預約中"?"selected":""}>預約中</option><option ${p.status==="已攻略"?"selected":""}>已攻略</option><option ${p.status==="未攻略"?"selected":""}>未攻略</option></select>`:"";const tmpl=edit?`<button onclick="fillTemplate('${p.id}')">套用模板</button>`:"";const d=document.createElement("div");d.className="card";d.id="card-"+p.id;d.innerHTML=`<div class="thumb ${bgClass(p.status)}">${statusIcon(p.status)}</div><div class="cardBody"><span class="pill ${pill(p.status)}">${p.status}</span> ${sel}<h3>${p.id}. ${p.name}</h3><div class="meta">📍 ${p.address}<br>☎️ ${p.phone||"待補"}<br>🐰 ${p.rabbit_note||""}</div></div><div class="actions"><a class="link" href="${p.google_maps}" target="_blank">Google 地圖</a><a class="link" href="${p.source}" target="_blank">來源</a>${tmpl}<button onclick="focusMarker('${p.id}')">定位圖釘</button></div>`;$("cards").appendChild(d)})}
...
function template(p){const done=countStatus("已攻略");return `🐰 培培兔子友善餐廳攻略 #${String(p.id).padStart(3,"0")}\n\n📍地點：${p.name}\n🗺️城市：${p.city}\n✅狀態：${p.status}\n🐰培培反應：\n⭐兔友善感受：\n🏍️路線心得：\n\n目前進度：${done} / ${places.length}\n#培培環台計畫 #兔子友善餐廳 #兔兔日常`}
